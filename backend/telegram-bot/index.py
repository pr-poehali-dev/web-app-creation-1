    ...
    try:
        ...
        if text.startswith('/start'):
            ...
            response_text = (
                ...
                f"1️⃣ Скопируйте Chat ID выше (нажмите на число)\n"
                f"2️⃣ Откройте сайт ЕРТТП → Профиль → Telegram уведомления\n"
                f"3️⃣ Вставьте Chat ID и нажмите \"Подключить\"\n\n"
                f"💡 Для удобства можете открыть сайт по кнопке ниже:"
            )
            ...
            # Создаем inline кнопку для перехода на сайт (без параметров, чтобы избежать проблем с mobile)
            keyboard = {
                'inline_keyboard': [[
